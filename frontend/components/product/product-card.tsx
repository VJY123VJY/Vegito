...
export function ProductCard({ product, quantity = 0, onChange, onLoginRequired }: { product: ApiProduct; quantity?: number; onChange?: (amount: number) => void; onLoginRequired?: () => void }) {
  ...
  return <article className="product-card">
    <div className="product-image product-art"><Link href={`/products/${product.id}`} aria-label={`View ${product.name}`}><span aria-hidden="true">{art[product.name.toLowerCase()] ?? "🥕"}</span>{available ? <small className="fresh-tag">Fresh today</small> : <small className="fresh-tag muted-tag">Availability soon</small>}</Link><button type="button" className="heart" disabled={favoriteMutation.isPending} aria-label={isFavorite ? `Remove ${product.name} from favorites` : `Add ${product.name} to favorites`} onClick={() => { if (role !== "CUSTOMER") { onLoginRequired?.(); return; } favoriteMutation.mutate(); }}><Heart size={18} fill={isFavorite ? "currentColor" : "none"} /></button></div>
    <div className="product-info"><Link href={`/products/${product.id}`} className="product-name">{product.name}</Link><p className="unit">{offer?.seller_business_name ?? "Local seller"} · {product.unit}</p><div className="product-bottom"><p className="price">{price == null ? <b className="awaiting-price">Price soon</b> : <><b>₹{price}</b><span>/{product.unit}</span></>}</p>{available && quantity && onChange ? <QuantitySelector quantity={quantity} onChange={onChange} /> : <button className="add-button" disabled={!available} onClick={action}>{available ? <><Plus size={18} /> Add</> : "Unavailable"}</button>}</div></div>
  </article>;
}
