...
export function OrderChart({
  ...
  subtitle = "Orders placed over selected duration",
  onRangeChange,
  selectedRange = "30d",
  isLoading = false,
}: OrderChartProps) {
  ...
  const chartData = (data || []).map((d) => ({
    ...
    orders: Number(d.value || d.orders_count || 0),
  }));
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ margin: "2px 0 0", fontSize: "12px", color: "#62746a" }}>
            {subtitle} · Total: <strong style={{ color: "#063c32" }}>{totalOrders} orders</strong>
          </p>
        </div>
      ...
      {isLoading ? (
        <div style={{ height: "260px", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <p style={{ margin: 0, fontSize: "13px", color: "#8b9c92" }}>Loading orders data...</p>
        </div>
      ) : chartData.length === 0 ? (
        <div style={{ height: "260px", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <p style={{ margin: 0, fontSize: "13px", color: "#8b9c92" }}>No orders placed in this period</p>
        </div>
      ) : (
        <div style={{ width: "100%", height: 260 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  if (active && payload && payload.length) {
                    return (
                      ...
                      >
                        ...
                        <p style={{ margin: 0, color: "#6ee7b7", fontWeight: 800 }}>Orders: {payload[0].value}</p>
                      </div>
                    );
                  }
                  return null;
              ...
              />
              <Bar dataKey="orders" fill="#063c32" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
