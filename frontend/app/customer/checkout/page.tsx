...
export default function CheckoutPage() {
  ...
  return <main className="simple-page cart-page"><Link className="back-link" href="/customer/cart"><ArrowLeft size={16} /> Back to basket</Link><h1>Checkout</h1><p className="helper">Delivery is currently available in Solapur.</p>
    ...
    <section className="checkout-panel"><p className="section-kicker">DELIVERY SLOT</p><label className="select-label">Choose a window<select value={slot} onChange={(event) => setSlot(event.target.value)}><option value="morning">Today, 8:00 AM - 12:00 PM</option><option value="today-evening">Today, 4:00 PM - 8:00 PM</option></select></label></section>
    <section className="cart-total"><span><ShoppingBasket size={16} /> Items <b>₹{Number(cart.data.subtotal).toFixed(2)}</b></span><span>Delivery <b>₹{Number(cart.data.delivery_charge).toFixed(2)}</b></span><strong>Total <b>₹{Number(cart.data.total_amount).toFixed(2)}</b></strong><small>Payment method: Cash on delivery</small><button className="primary-action" disabled={!selectedAddress || placeOrder.isPending} onClick={() => { setError(""); placeOrder.mutate(); }}>{placeOrder.isPending ? "Placing order..." : "Place COD order"}</button></section>
